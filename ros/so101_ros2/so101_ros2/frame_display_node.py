...
Renders fullscreen via cv2.imshow (GTK3 Wayland backend).
On Desktop: connects to the GNOME Wayland session.
On Ubuntu Core: connects to ubuntu-frame (the built-in Wayland compositor).
Both paths use the snap's `wayland` plug — no separate kiosk snap needed.
"""
...
class FrameDisplayNode(Node):
    ...
    def __init__(self):
        ...
        try:
            ...
            self.get_logger().info(
                f'FrameDisplayNode ready — fullscreen {self._win_w}x{self._win_h}')
        except cv2.error as e:
            self.get_logger().warn(
                f'No display available ({e}); running headless. '
                'Ensure the wayland plug is connected and WAYLAND_DISPLAY is set.'
            )
    ...
    def _display_cb(self):
        if not self._has_display:
            return
        # Adapt to compositor-driven window resize (ubuntu-frame may resize us)
...
def main(args=None):
    # Force GTK3 to use the Wayland backend so cv2.imshow renders via Wayland.
    # On Desktop: connects to GNOME/Wayland session.
    # On Ubuntu Core: connects to ubuntu-frame (the built-in Wayland compositor).
    # The snap's `wayland` plug provides access to $XDG_RUNTIME_DIR/wayland-0.
